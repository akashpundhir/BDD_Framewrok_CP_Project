...
namespace Multisite.Steps
{
    ...
    public class SignUpSteps
    {
        ...
        public void WhenIClickRegister()
        {
            ...
            try
            {
                Driver.Browser.FindElement(By.XPath("//span[contains(., 'Register')]"), 20).WaitForElementToBeClickable().Click();
            }
            catch
            {
        ...
        public void ThenNewAccountIsCreated()
        {
            Driver.BrowserWait.WaitForPageLoad();
            Thread.Sleep(5000);
            var completeProfileBtn = Driver.Browser.FindElement(By.XPath("//div[contains(@class, 'modal')]//h2[contains(.,'Account created')]"), 60);
